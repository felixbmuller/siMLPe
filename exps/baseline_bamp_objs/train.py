...
from datasets.bamp import SittingDataset, person_seqs, kitchens
...
def train_step(h36m_motion_input, h36m_motion_target, model, optimizer, nb_iter, total_iter, max_lr, min_lr) :
    ...
    if config.use_relative_loss:
        motion_pred = motion_pred.reshape(b,n,29,3)
        dmotion_pred = gen_velocity(motion_pred)
        motion_gt = h36m_motion_target.reshape(b,n,29,3)
...
#eval_dataset = SittingDataset(person_seqs, kitchens, ["D"], config.motion.h36m_input_length, config.motion.h36m_target_length)
